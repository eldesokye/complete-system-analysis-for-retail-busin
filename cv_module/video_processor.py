...
class VideoProcessor:
    ...
    def process_frame(self, frame: np.ndarray, source_name: str, role: str) -> Dict:
        ...
        """
        # Count people
        count, boxes = self.people_counter.count_people(frame)
        
        # Classify gender
        gender_data = self.gender_classifier.classify_gender(frame, boxes)
        
        # Update heatmap
        centers = self.people_counter.get_person_centers(boxes)
        ...
        if role == "cashier" and self.queue_detector:
            queue_length = self.queue_detector.detect_queue(boxes)
